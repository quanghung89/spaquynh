...
class Auth extends MY_Controller
{
    ...
    function getTimeout()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("1,sma_timeout.sma_timeout_id,sma_timeout.sma_timeout_userid, sma_timeout.sma_timeout_startdate,sma_timeout.sma_timeout_enddate,sma_timeout.sma_timeout_day")
            // ->join('sma_users','sma_users.id = sma_bouns.sma_bouns_createby','left')
            ->from('sma_timeout');
    ...
    function profile($id = NULL)
    {
        ...
        $this->data['timeouts'] = $this->site->getAllTimeoutByIDUser($id);
   ...
   function update_bouns($id = NULL,$idbouns = NULL)
    ...
    function time_out($id){
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            $data = array(
                'sma_timeout_startdate' => strtotime($this->sma->fsd($this->input->post('start_date'))),
                'sma_timeout_enddate' => strtotime($this->sma->fsd($this->input->post('end_date'))),
