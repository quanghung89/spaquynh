...
class Customers extends MY_Controller
{
    ...
    function book($id = NULL,$change)
    {
        ...
        if($this->form_validation->run() == true){
            ...
            $user = $this->site->getUser();
            if($user->warehouse_id != 'all'){
                $this->data['warehouses'] = $this->site->getWarehouseByID($user->warehouse_id);
            }else{
    ...
    function getStaff(){
        ...
        if($this->input->post('id_book')){
            ...
            $user = $this->site->getAllUserByWarhouseID($this->input->post('warehouse_id'),1);
            foreach ($user as $key => $value) { 
                ...
                if($data_book->sma_books_starttime <= strtotime($this->sma->fld($this->input->post('time'))) && strtotime($this->sma->fld($this->input->post('time'))) <= $data_book->sma_books_endtime){
                    ...
                    $detail = $this->sma->getUserWork($value['id'],$time1);
                }
